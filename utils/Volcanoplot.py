...
def preprocess_volcano(df_volc):
    ...
    try:
        ...
        df_volc.reset_index(inplace=True,drop=True)
...
def map_color(a):
    ...
    if log2FoldChange > 1 and nlog10 > 1.30:
        return 'upregulated'
    elif log2FoldChange < -1 and nlog10 > 1.30:
...
def volcano_plot(df_volc,path_save = False, texto =None,show=True):
    ...
    ax.axhline(1.30, zorder = 0, c = 'k', lw = 2, ls = '--')
