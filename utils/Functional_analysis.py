...
def generate_cls(condition,cls_path="./Data/gsea.cls"):
    ...
    """
    if np.all(np.sort(condition)[::-1]==condition) or np.all(np.sort(condition)==condition):
...
def sig_finder(gs_res,threshold=0.05,n_top_bottom=10):
    ...
    if len(sig_res2d)>n_top_bottom+1:
        sig_res2d = pd.concat([sig_res2d.sort_values(by="fdr").reset_index().loc[:n_top_bottom],
                               sig_res2d.sort_values(by="fdr").reset_index().loc[-n_top_bottom:]])
...
def run_ora(genes,datasets,save_path=False,figsize=(3,7),organism='human',size=6,top_terms=5):
